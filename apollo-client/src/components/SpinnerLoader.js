...
const Pacman = styled("div")`
  ...
  animation: ${({ version }) => pacman[version]} ease-in-out 0.8s infinite
    normal both running;
`;
...
const Ball = styled("div")`
  ...
  animation: ${({ size, sizeUnit, version }) =>
    `${ballAnim(size, sizeUnit)} 1s linear ${version *
      0.25}s infinite normal both running`};
`;
...
export const SpinnerLoader = {
  ...
  render(h, { props, data }) {
    return props.loading ? (
      <Wrapper {...data} size={props.size} sizeUnit={props.sizeUnit}>
        <Pacman
          color={props.color}
          size={props.size}
